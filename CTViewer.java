...
public class CTViewer extends JFrame implements ActionListener, WindowListener {
	...
	private List<HUnit> ctData;
	private BufferedImage ctImage;
	private byte[] ctImageData;
	private HWindow ctWindow;

	private void initCT() {
		this.ctWindow = new HWindow();
	...
	public CTViewer() {
		...
		this.levelTextField = makeTextField(HUnit.MIN_VALUE, HUnit.MAX_VALUE, 
				this.ctWindow.level());
		...
		this.widthTextField = makeTextField(0, HUnit.MAX_VALUE - HUnit.MIN_VALUE, 
				this.ctWindow.width());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.open) {
			...
			try {
				...
				this.ctWindow.level(level);
				this.ctWindow.width(width);
				this.updateImage();
			}
	...
	private void readCT(File f) {
		...
		try {
			...
			List<HUnit> ctNew = new ArrayList<>(512 * 512);
			while (values.hasRemaining()) {
				HUnit hv = new HUnit(values.get());
				ctNew.add(hv);
			}
	...
	private void updateImage() {
		...
		int i = 0;
		for (HUnit hv : this.ctData) {
			byte gray = (byte) (this.ctWindow.mapLinear(hv) * 255.0);
			b.setElem(i++, gray);
		}
